...
use App\Http\Requests\Admin\ZGalleryRequest;
use App\Http\Requests\Admin\ZGalleryTypeRequest;
use App\Models\ZGallery;
...
/**
 * Class ZGalleryTypeController
 * @package App\Http\Controllers\Admin
 * @property ZGallery $model
 */
class ZGalleryController extends BaseController
{
    use HasLocaleTrait;
    ...
    {
        $this->model = new ZGallery();
        $this->slug = 'z_galleries';
    }
    ...
    public function getTableData(Request $request)
    {
        return DataTables::of($this->model::getEloquentList($request->all()))
            ->escapeColumns([])
            ->editColumn("z_gallery_type_id", function ($data) {
                return ($zGalleryType = $data->zGalleryType) ? $zGalleryType->global_name : "";
            })
    ...
    {
        $data = $this->model;
        $zGalleryTypes = ZGalleryType::getList();

        $compactParams = array_merge(
            compact("data", "zGalleryTypes"),
    ...
    public function store(ZGalleryRequest $request)
    {
        try {
    ...
    public function update(ZGalleryRequest $request, $id)
    {
        try {
            if (!$data = $this->model::getById($id)) {
                return abort(Helper::HTTP_NOT_FOUND);
