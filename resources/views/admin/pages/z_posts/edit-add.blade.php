...
@section("pageTitle", __("admin_global.label_". ($editFlg ? "edit" : "create")). " " . __("admin_table.z_posts.label_title"))

@section("mainContent")
    <form class="js_server_form"
          action="{{ $editFlg ? route("admin.z_posts.update", $data->getKey()) : route("admin.z_posts.store") }}"
          method="POST"
        ...
        <div class="row">
            <div class="col-md-9">
                <div class="row" id="js_edit_content_on_change_language">
                    @include("admin.pages.z_posts.edit-add-locale", ["data" => $dataTranslation])
                </div>
            </div>
            <div class="col-md-3">
                <div class="edit-add-action-wrapper">
                    @include("admin.actions.button-cancel", ["url" => route("admin.z_posts.index")])
                    @include("admin.actions.button-save")
                </div>

                @include("admin.includes.language-id-edit-add-area",[
                    'data' => $data,
                    "routeName" => "admin.z_posts.get_data_language",
                    "languages" => $languages
                ])
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label
                                        for="type">{{ __("admin_table.z_posts.attr_type") }}</label>
                                    <select class="form-control select2" id="type" name="type"
                                            style="width: 100%;">
                                        @foreach($types as $item)
                                            <option value="{{ $item }}"
                                                    @if($item == old("type", $data->type)) selected @endif>
                                                {{ __("admin_table.z_posts.option_type_". $item) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
