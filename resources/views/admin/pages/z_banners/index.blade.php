...
@section("pageTitle", __("admin_table.z_banners.label_title"))

@section("mainContent")
    <div class="card">
        <div class="card-header">
            @include("admin.actions.button-create", ["url" => route("admin.z_banners.create")])
        </div>

        <div class="card-body">
            <div class="table-responsive">
                <table class="table table-bordered table-striped js_datatable"
                       data-url="{{ route("admin.z_banners.get_table_data") }}"
                       data-cols="id,global_name,type,type_model_id,home_flg,active_flg,created_at,action_edit,action_delete,action_clone"
                >
                    <thead>
                    <tr>
                        <th>ID</th>
                        <th>{{ __("admin_global.attr_global_name") }}</th>
                        <th>{{ __("admin_table.z_banners.attr_type") }}</th>
                        <th>{{ __("admin_table.z_banners.attr_type_model_id") }}</th>
