...
@section("pageTitle", __("admin_global.label_". ($editFlg ? "edit" : "create")). " " . __("admin_table.z_news_types.label_title"))

@section("mainContent")
    <form class="js_server_form"
          action="{{ $editFlg ? route("admin.z_news_types.update", $data->getKey()) : route("admin.z_news_types.store") }}"
          method="POST"
        ...
        <div class="row">
            <div class="col-md-9">
                <div class="row" id="js_edit_content_on_change_language">
                    @include("admin.pages.z_news_types.edit-add-locale", ["data" => $dataTranslation])
                </div>
            </div>
            <div class="col-md-3">
                <div class="edit-add-action-wrapper">
                    @include("admin.actions.button-cancel", ["url" => route("admin.z_news_types.index")])
                    @include("admin.actions.button-save")
                </div>

                @include("admin.includes.language-id-edit-add-area",[
                    'data' => $data,
                    "routeName" => "admin.z_news_types.get_data_language",
                    "languages" => $languages
                ])
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label
                                        for="parent_id">{{ __("admin_table.z_news_types.attr_parent_id") }}</label>
                                    <select class="form-control select2" id="parent_id" name="parent_id"
                                            style="width: 100%;">
                                        <option value=""></option>
                                        @foreach($zNewsTypes as $item)
                                            <option value="{{ $item->getKey() }}"
                                                    @if($item->getKey() == old("parent_id", $data->parent_id)) selected @endif>
                                                {{ $item->global_name }}
                                            </option>
                                        @endforeach
                                    </select>
