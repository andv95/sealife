...
@section("pageTitle", __("admin_global.label_". ($editFlg ? "edit" : "create")). " " . __("admin_table.z_teams.label_title"))

@section("mainContent")
    <form class="js_server_form"
          action="{{ $editFlg ? route("admin.z_teams.update", $data->getKey()) : route("admin.z_teams.store") }}"
          method="POST"
        ...
        <div class="row">
            <div class="col-md-9">
                <div class="row" id="js_edit_content_on_change_language">
                    @include("admin.pages.z_teams.edit-add-locale", ["data" => $dataTranslation])
                </div>
            </div>
            <div class="col-md-3">
                <div class="edit-add-action-wrapper">
                    @include("admin.actions.button-cancel", ["url" => route("admin.z_teams.index")])
                    @include("admin.actions.button-save")
                </div>

                @include("admin.includes.language-id-edit-add-area",[
                    'data' => $data,
                    "routeName" => "admin.z_teams.get_data_language",
                    "languages" => $languages
                ])
                ...
                <div class="card card-default">
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="col-md-12">
                                <div class="form-group">
                                    <label for="branch">{{ __("admin_table.z_teams.attr_branch") }}</label>
                                    <select class="form-control select2" id="branch" name="branch"
                                            style="width: 100%;">
                                        @foreach($branches as $item)
                                            <option value="{{ $item }}"
                                                    @if($item == old("branch", $data->branch)) selected @endif>
                                                {{ __('admin_table.z_teams.option_branch_'. $item) }}
                                            </option>
                                        @endforeach
                                    </select>
                                </div>
